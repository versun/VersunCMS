  ...
  class CrosspostsController < ApplicationController
    ...
    def update
      ...
      if @settings.update(crosspost_params)
        ...
        redirect_to tools_crossposts_path, notice: "CrossPost settings updated successfully."
      else
        Rails.logger.error "Failed to update Crosspost: #{@settings.errors.full_messages}"
        redirect_to tools_crossposts_path, alert: @settings.errors.full_messages.join(", ")
      end
    end
    ...
    def verify
      ...
      begin
        ...
        when "mastodon"
          MastodonService.new(nil).verify(crosspost)
        when "twitter"
          TwitterService.new(nil).verify(crosspost)
        when "bluesky"
          # Set default server_url if not provided
          crosspost[:server_url] = "https://bsky.social/xrpc" if crosspost[:server_url].blank?
          BlueskyService.new(nil).verify(crosspost)
        else
